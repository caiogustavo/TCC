...
int main(int argc, char *argv[])
{
	...
	while((c = getopt(argc, argv, "g:io:p:q:h")) != -1) {
		switch(c) {
		case 'g':
		...
		case 'o':
			direction=OUT;
			valuein8=(char)strtoul(optarg,NULL,16);
			size = 8;
			break;
		case 'p':
			direction=OUT;
			size = 16;
			valuein16=(unsigned short int)strtoul(optarg,NULL,16);
			break;
		case 'q':
			direction=OUT;
			size = 32;
			valuein32=(unsigned)strtoul(optarg,NULL,16);
			break;
		case 'h':
			usage();
			return 0;
	...
	/* Read value from the device register */
		valueout = *((unsigned *)(ptr + page_offset));
		printf("gpio dev-mem test: input: %08x\n",valueout);
	} else if(size == 8) {
	/* Write value to the device register */
		*((char *)(ptr + page_offset)) = valuein8;
	} else if(size == 16) {
		*((unsigned short int *)(ptr + page_offset)) = valuein16;
	} else if( size == 32){
		*((unsigned *)(ptr + page_offset)) = valuein32;
	}
